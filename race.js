	...
	self.startRound = function(players, sockets)
	{
		...
		for (let i in players)
		{
			if (players[i].canRace)
			{
				...
				sockets[i].emit('getReady', {roundNumber : self.currentRound});
			}
		}
		...
		if (self.playersFinished == Object.keys(self.stats).length)
		{
		  ...
		  if (self.numOfRounds < self.currentRound)
			...
			for (let i in players)
			{
				if (players[i].canRace)
				{
					sockets[i].emit('displayResults', {stats : self.stats, isComplete : finished, numOfRounds : self.numOfRounds, roundNumber : self.currentRound - 1});
				}
			}
